...
- **Blade regression safety rule (mandatory)**: Every time a Blade file is modified (`resources/views/**/*.blade.php`), create a focused Dusk Browser test first (or extend an existing equivalent Dusk test) for the affected UI flow before continuing with the Blade change, and run that Dusk test before finalizing to verify no UI regression.
...
- **No hidden dependencies (mandatory)**: Do not introduce new hidden dependencies in business logic via service locator calls (for example `app(...)` / `resolve(...)`). Prefer explicit constructor or method injection so dependencies are visible in method signatures and easier to test.
