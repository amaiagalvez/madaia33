...
test('bilingual settings blocks keep the same height when switching between EUS and CAS', function () {
  ...
  Setting::query()->updateOrCreate(
    ['key' => 'admin_email'],
    ['value' => 'admin@madaia33.eus', 'section' => Setting::SECTION_CONTACT_FORM],
  );
    ...
    $browser->loginAs($admin)
      ...
      ->assertScript(<<<'JS'
                (() => {
                    ...
                    root.querySelector('[data-bilingual-tab="eu"]').click();
                    const euHeight = Math.round(root.getBoundingClientRect().height);

                    root.querySelector('[data-bilingual-tab="es"]').click();
