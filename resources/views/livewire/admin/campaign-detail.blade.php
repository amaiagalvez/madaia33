<div class="space-y-6" data-campaign-detail-page>
    ...
    <div class="rounded-xl border border-[#edd2c7] bg-[#edd2c7]/25 p-4">
        ...
        <p class="mt-1 text-sm text-stone-700">{{ $campaign->subject_eu ?? $campaign->subject_es ?? '—' }}</p>
        <p class="mt-3 text-xs text-stone-600">{{ __('campaigns.admin.channel') }}: {{ __('campaigns.admin.channels.' . $campaign->channel) }}</p>
    </div>

    <x-admin.panel-table table-class="min-w-full divide-y divide-gray-200" data-campaign-recipient-detail-table>
            ...
            @forelse ($recipientRows as $row)
                <tr wire:key="campaign-detail-row-{{ $row['id'] }}">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600">
                        <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold {{ $row['status'] === 'failed' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                            {{ $row['status_label'] }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-600">
                    ...
                    <td class="px-6 py-4 text-right text-sm font-medium">
                        <x-admin.action-link-confirm wire:click="toggleRecipientDetails({{ $row['id'] }})"
                ...
                @if ($expandedRecipientId === $row['id'])
                    <tr wire:key="campaign-detail-events-{{ $row['id'] }}" class="bg-stone-50">
                        <td colspan="8" class="px-6 py-4">
                            <div class="space-y-3 rounded-lg border border-stone-200 bg-white p-4" data-campaign-event-list>
                                <h4 class="text-sm font-semibold text-stone-900">{{ __('campaigns.admin.event_details') }}</h4>

                                @forelse ($row['events'] as $event)
                                    <div class="rounded-md border border-stone-200 px-3 py-2 text-sm text-stone-700">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <span class="font-semibold text-[#793d3d]">{{ $event['type_label'] }}</span>
                                            <span class="text-xs text-stone-500">{{ $event['created_at']?->format('d/m/Y H:i') ?? '—' }}</span>
                                            @if ($event['ip_address'])
                                                <span class="text-xs text-stone-500">IP: {{ $event['ip_address'] }}</span>
                                            @endif
                                        </div>

                                        @if ($event['url'])
                                            <p class="mt-1 break-all text-xs text-stone-600">{{ $event['url'] }}</p>
                                        @endif
                                    </div>
                                @empty
                                    <p class="text-sm text-stone-500">{{ __('campaigns.admin.no_events') }}</p>
                                @endforelse
                            </div>
                        </td>
                    </tr>
