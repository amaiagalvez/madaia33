<div>
    ...
    <x-admin.panel-table table-class="min-w-full divide-y divide-gray-200 text-sm"
        class="overflow-x-auto" data-campaign-table>
        <thead class="bg-gray-50">
            <tr>
                ...
                <x-admin.table-header-cell>{{ __('campaigns.admin.opened_messages_count') }}</x-admin.table-header-cell>
                <x-admin.table-header-cell sortable
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
            @forelse ($campaigns as $campaign)
                <tr wire:key="campaign-row-{{ $campaign->id }}">
                    ...
                    <td class="px-2 py-2 text-sm text-gray-600 max-w-40 truncate"
                        title="{{ $campaign->locations->isNotEmpty()
                    ...
                    <td class="px-2 py-2 text-sm text-gray-600 whitespace-nowrap text-center">
                        {{ $campaign->recipients_count }}
                    </td>
                    <td class="px-2 py-2 text-sm text-gray-600 whitespace-nowrap text-center">
                        {{ $campaign->opened_recipients_count }}
                    </td>
                    <td class="px-2 py-2 text-sm text-gray-600 whitespace-nowrap">
                        ...
                        <x-admin.table-row-actions :bars-href="route('admin.campaigns.show', $campaign)" :bars-title="__('campaigns.admin.events')"
                            ...
                            @if ($campaign->status === 'draft')
                                <x-admin.action-link-confirm
                                    ...
                                    class="!p-1 !min-w-0" state="success">
                                    <flux:icon.paper-airplane class="size-4" />
                                </x-admin.action-link-confirm>

                                <x-admin.action-link-confirm
                                    wire:click="openScheduleModal({{ $campaign->id }})"
                                    title="{{ __('campaigns.admin.actions.schedule') }}"
                                    class="!p-1 !min-w-0" state="neutral"
                                    data-campaign-schedule-action="{{ $campaign->id }}">
                            ...
                            @if ($campaign->status === 'scheduled')
                                <x-admin.action-link-confirm
                                    ...
                                    class="!p-1 !min-w-0" state="danger">
    ...
    @if ($showScheduleModal)
        <dialog open
            class="fixed inset-0 z-50 m-0 grid h-full w-full place-items-center bg-transparent p-4"
            aria-labelledby="campaign-schedule-modal-title" data-campaign-schedule-modal>
            <div class="mx-4 w-full max-w-sm space-y-4 rounded-xl bg-white p-6 shadow-2xl">
                <div class="flex items-start gap-3">
                    <div
                        class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-100">
                    ...
                    <div>
                        <h3 id="campaign-schedule-modal-title"
                            class="text-base font-semibold text-gray-900">
                            {{ __('campaigns.admin.schedule_modal.title') }}
                        </h3>
                        <p class="mt-1 text-sm text-gray-600">
                            {{ __('campaigns.admin.schedule_modal.description') }}
                        </p>
                    </div>
                </div>

                <div>
                    <label for="campaign-schedule-at"
                        class="block text-sm font-medium text-stone-700">
                        {{ __('campaigns.admin.scheduled_at') }}
                    </label>
                    <input id="campaign-schedule-at" type="datetime-local"
                        wire:model.defer="scheduleAtInput"
                        class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-stone-900 shadow-sm focus:border-brand-600 focus:outline-none focus:ring-1 focus:ring-brand-600"
                        data-campaign-schedule-input />

                    @error('scheduleAtInput')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>

                <div class="flex justify-end gap-3">
                    <button type="button" wire:click="cancelScheduleModal"
                        class="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-brand-600"
                        data-campaign-schedule-cancel>
                        {{ __('general.buttons.cancel') }}
                    </button>
                    <button type="button" wire:click="saveSchedule"
                        class="rounded-md bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-400"
                        data-campaign-schedule-save>
                        {{ __('campaigns.admin.schedule_modal.save') }}
                    </button>
                </div>
            </div>
        </dialog>
