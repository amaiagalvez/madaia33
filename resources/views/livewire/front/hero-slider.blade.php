    ...
    @else
        ...
        <div class="relative w-full h-full overflow-hidden bg-stone-950">
            @foreach ($images as $index => $image)
                @php
                    $rawAltText = (string) ($image['alt_text'] ?? __('hero_slider.gallery_title'));
                    $heroAltText = trim(
                        (string) preg_replace('/\b(image|imagen|irudia)\b/iu', '', $rawAltText),
                    );
                    $heroAltText =
                        $heroAltText !== '' ? $heroAltText : __('hero_slider.gallery_title');
                @endphp
                <div class="absolute inset-0 transition-opacity duration-500 ease-in-out"
                    ...
                    <div class="flex h-full w-full items-center justify-center">
                        <img src="{{ Storage::disk('public')->exists($image['path']) ? Storage::url($image['path']) : asset('favicon.svg') }}"
                            alt="{{ $heroAltText }}" class="h-full w-full object-contain"
