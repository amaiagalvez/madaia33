<div data-test="profile-contact-modal" x-data="{
    open: @js($showModal),
    openModal() {
        this.open = true;
    ...
    closeModal() {
        this.open = false;
        this.$nextTick(() => this.$refs.triggerButton?.focus());
    },
}"
    x-on:profile-contact-modal-opened.window="openModal()"
    x-on:profile-contact-modal-closed.window="closeModal()">
    {{-- Trigger button --}}
    ...
    <template x-teleport="body">
        <div x-cloak x-show="open" x-transition.opacity
            class="fixed inset-0 z-120 overflow-y-auto p-4 sm:p-6" aria-modal="true" role="dialog"
            aria-labelledby="profile-contact-modal-title"
            aria-describedby="profile-contact-modal-description"
            x-on:keydown.escape.window="$wire.close()" data-test="profile-contact-modal-dialog">

            {{-- Backdrop --}}
            <div class="fixed inset-0 bg-black/50" wire:click="close" @click="closeModal()"
                aria-hidden="true"></div>

            <div class="relative z-10 flex min-h-full items-center justify-center">
                {{-- Panel --}}
                <div
                    class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
                    ...
                    <div class="mb-4 flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p id="profile-contact-modal-description"
                                class="mt-1 text-sm text-gray-500">
                                {{ __('profile.contact_modal.description') }}
                            </p>
                        </div>
                        <button type="button" wire:click="close" @click="closeModal()"
                            x-ref="closeButton"
                    ...
                    <form wire:submit="submit" novalidate>
                        <div class="mb-4">
                            ...
                            <textarea id="profile-contact-message" wire:model="message" rows="5" maxlength="5000"
                                ...
                                @if ($errors->has('message')) aria-describedby="profile-contact-message-error" @endif
                        ...
                        <div class="flex items-center justify-end gap-3">
                            <button type="button" wire:click="close" @click="closeModal()"
                                class="inline-flex min-h-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-brand-600 hover:text-[#793d3d]">
                                {{ __('general.buttons.cancel') }}
                            </button>
                            <button type="submit" data-test="profile-contact-modal-submit"
                                class="inline-flex min-h-10 items-center justify-center rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#793d3d]">
