    ...
    <div
        ...
        <div
            ...
            <a href="{{ route(\App\SupportedLocales::routeName('home')) }}"
                class="relative z-20 flex items-center text-lg font-medium" wire:navigate>
            ...
            <div class="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-87.5">
                <a href="{{ route(\App\SupportedLocales::routeName('home')) }}"
                    ...
                    <span class="flex h-9 w-9 items-center justify-center rounded-md">
                        <x-shared.app-logo-icon
                            class="size-9 fill-current text-black dark:text-white" />
                    </span>

                    <span
                        class="sr-only">{{ $publicSiteName ?? config('app.name', 'Laravel') }}</span>
                </a>
                {{ $slot }}
            </div>
        </div>
